...
public class SuffixTreePerformanceTest {
	...
	private SuffixTreePerformanceTest() throws Exception{
		...
		int stringSize = 10; 
		System.out.println("Length ("+multiplier+"'s), Time (s), Space Total (MB), Space Free(MB)");
		for(int i = 1; i < 11; i++){
			
			RandomString rs = new RandomString(stringSize*(i*multiplier));
			Character[] s = rs.nextString();
			
			long allocatedMemoryStart = runtime.totalMemory()-runtime.freeMemory();
		    
			long start = System.nanoTime();
			SuffixTree<Character> tree = new SuffixTree<Character>(s);
			long end = System.nanoTime()-start;
			
			long allocatedMemoryEnd = runtime.totalMemory()-runtime.freeMemory();
			
			long space = ((allocatedMemoryEnd-allocatedMemoryStart)/1024)/1024;
			double time = (((double)end/1000d)/1000d)/1000d;

			System.out.println(stringSize*i + ", " + time + ", " + (runtime.totalMemory()/1024)/1024 + ", " + (runtime.freeMemory()/1024)/1024);
